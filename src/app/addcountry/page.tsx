...
const UploadForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${apiUrl}/api/country/add`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      setMessage(response.data.message || 'Country added successfully!');
    } catch (error) {
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-8 mt-24 border border-gray-300 rounded-lg shadow-md">
      <h2 className="text-2xl font-semibold text-center mb-6">Add Country</h2>
